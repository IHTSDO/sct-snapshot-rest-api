    ...
    var processMatches = function(docs){
        ...
        if (docs && docs.length > 0) {
            ...
            if (filters.idParamStr == docs[0].descriptionId) {
                result.matches.push({"term": docs[0].term, "typeId":docs[0].typeId, "conceptId": docs[0].conceptId, "active": docs[0].active, "conceptActive": docs[0].conceptActive, "fsn": docs[0].fsn, "conceptModule": docs[0].conceptModule, "module": docs[0].module});
                callback(false, result);
            } else {
                ...
                matchedDescriptions.forEach(function(doc) {
                    ...
                    if (filters.semanticFilter == "none" || (filters.semanticFilter == doc.semanticTag)) {
                        if (filters.langFilter == "none" || (filters.langFilter == doc.lang)) {
                            if (filters.moduleFilter == "none" || (filters.moduleFilter == doc.conceptModule)) {
                                if (filters.refsetFilter == "none" || refsetOk) {
                                    if (!filters["groupByConcept"] || conceptIds.indexOf(doc.conceptId) == -1) {
                                        ...
                                        if (count >= filters.skipTo && count < (filters.skipTo + filters.returnLimit)) {
                                            if (filters["exclDefinitions"]) {
						if (doc.typeId != "900000000000550004") { 
	                                            result.matches.push({"term": doc.term, "typeId":doc.typeId, "conceptId": doc.conceptId, "active": doc.active, "conceptActive": doc.conceptActive, "fsn": doc.fsn, "module": doc.module, "conceptModule": doc.conceptModule, "definitionStatus": doc.definitionStatus});
						}
                                            } else {
                                               result.matches.push({"term": doc.term, "typeId":doc.typeId, "conceptId": doc.conceptId, "active": doc.active, "conceptActive": doc.conceptActive, "fsn": doc.fsn, "module": doc.module, "conceptModule": doc.conceptModule, "definitionStatus": doc.definitionStatus});
					    }
