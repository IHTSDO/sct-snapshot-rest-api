...
app.get('/v1/:db/:collection/concepts/:sctid?', function(req, res) {
...
app.get('/v1/:db/:collection/concepts/:sctid/descriptions/:descriptionId?', function(req, res) {
    var idParam = parseInt(req.params.sctid);
    var query = {'conceptId': idParam};
    var options = req.params.options || {};
    var test = ['limit', 'sort', 'fields', 'skip', 'hint', 'explain', 'snapshot', 'timeout'];
...
app.get('/v1/:db/:collection/concepts/:sctid/relationships?', function(req, res) {
    var idParam = parseInt(req.params.sctid);
    var query = {'conceptId': idParam};
    var form = "all";
    if (req.query["form"]) {
...
app.get('/v1/:db/:collection/concepts/:sctid/children?', function(req, res) {
    var idParam = parseInt(req.params.sctid);
...
app.get('/v1/:db/:collection/concepts/:sctid/parents?', function(req, res) {
    var idParam = parseInt(req.params.sctid);
    var query = {'conceptId': idParam};
    var options = req.params.options || {};
    var test = ['limit', 'sort', 'fields', 'skip', 'hint', 'explain', 'snapshot', 'timeout'];
...
app.get('/v1/:db/:collection/descriptions/:sctid?', function(req, res) {
    var idParam = null;
    ...
    if (req.params.sctid) {
        ...
        if (req.query["query"]) {
            if (!req.query["searchMode"] || req.query["searchMode"] == "startsWith") {
                ...
                query = {"term": {"$regex": wildcarded}, "active": true, "conceptActive": true};
            } else if (req.query["searchMode"] == "phraseMatch") {
                searchMode = req.query["searchMode"];
                searchTerm = req.query["query"];
                query = {"text": {"search": searchTerm, "limit": 100, filter: {"active": true}}};
            } else if (req.query["searchMode"] == "wordsAnyOrder") {
                searchMode = req.query["searchMode"];
                searchTerm = req.query["query"];
    ...
    if (searchMode == "startsWith" || searchMode == "wordsAnyOrder")  {
        var db = new mongo.Db(req.params.db, new mongo.Server(config.db.host, config.db.port, {'auto_reconnect': true}), {safe:false});
        db.open(function(err, db) {
            db.authenticate(config.db.username, config.db.password, function() {
                                    ...
                                    matchedDescriptions.forEach(function(doc) {
                                        if (count < returnLimit) {
                                            if (semanticFilter == "none" || (semanticFilter == doc.semanticTag)) {
                                                result.push({"term": doc.term, "conceptId": doc.conceptId, "active": doc.active, "fsn": doc.fsn});
                                                count = count + 1;
                                            }
                                        }
                                    });
                                    var duration = Date.now() - start;
                                    logger.log('info', 'Search for ' + searchTerm + ' result = ' + docs.length, {searchTerm: searchTerm, database: req.params.db, collection: req.params.collection, searchMode: searchMode, language: lang, matches: docs.length, duration: duration});
                                    res.header('Content-Type', 'application/json');
                                    res.send(result);
                                }
                            } else {
    ...
    } else if (searchMode == "phraseMatch") {
        var db = new mongo.Db(req.params.db, new mongo.Server(config.db.host, config.db.port, {'auto_reconnect': true}), {safe:false});
        db.open(function(err, db) {
            db.authenticate(config.db.username, config.db.password, function() {
                db.command({text: req.params.collection + 'tx', search: searchTerm, language: lang, filter: {"active": true, "conceptActive": true}}, function(e, cb) {
                    if (e) {
                        console.log(e, 'error');
                        var duration = Date.now() - start;
                        logger.log('error', 'Search for ' + searchTerm + ' ERROR', {searchTerm: searchTerm, database: req.params.db, collection: req.params.collection, searchMode: searchMode, language: lang, duration: duration});
                        res.send(501);
                    } else {
                        ...
                        if (matchedDescriptions.length > 0) {
                            ...
                            matchedDescriptions.forEach(function(doc) {
                                if (count < returnLimit) {
                                    result.push({"term": doc.obj.term, "conceptId": doc.obj.conceptId, "active": doc.obj.active, "fsn": doc.obj.fsn});
                                    count = count + 1;
                                }
                            });
                            var duration = Date.now() - start;
                            logger.log('info', 'Search for ' + searchTerm + ' result = ' + cb.results.length, {searchTerm: searchTerm, database: req.params.db, collection: req.params.collection, searchMode: searchMode, language: lang, matches: cb.results.length, duration: duration});
                            res.header('Content-Type', 'application/json');
                            res.send(result);
                        } else {
                            var duration = Date.now() - start;
