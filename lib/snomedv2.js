            ...
            if (err) callback(err);
                ...
                if (docs && docs.length > 0) {
                    if (!docs[0].v || docs[0].v != "2") {
                        ...
                        result.members = docs;
                        console.log("step getmem 8");

                        if (v1 ) {

                            console.log("step getmem 9");
                            getDefaultTermType(dbP, collectionP, function(err,defTermType) {
                                console.log("step getmem 10");
                                if (err) callback(err);
                                else {
                                    console.log("step getmem 11");
                                    result.members = transform.getV1ConceptDescriptors(result.members, defTermType);

                                    console.log("step getmem 12");
                                    callback(false, result);
                                }
                            });
