    ...
    getObject(dbP, collectionP, {'conceptId': conceptId }, options, function(err, docs){
        if (err) callback(err);
            ...
            if (!docs[0].v || docs[0].v!="2") {
                callback(true, "The db isnot version 2. It must be created with the new conversion module.");

            }else {
                if (options.v1) {
                    var result = transform.getV1Concept(docs[0]);
        ...
        if (err) callback(err);
            ...
            if (!doc.v || doc.v!="2") {
                ...
                if (doc.relationships) {
                    ...
                    if (options.v1) {
                        result = transform.getV1Relationships(result);
                    }
                }
                callback(false, result);
            }
        ...
        if (err) callback(err);
            ...
            if (!doc.v || doc.v!="2") {
                ...
                if (typeof doc.relationships != 'undefined') {
                    ...
                    if (options.v1 && result.length>0) {
                        result = transform.getV1ConceptDescriptors(result);
                    }
                }
                callback(false, result);
            }
            ...
            if (err) callback(err);
                ...
                if (docs && docs.length > 0) {
                    ...
                    if (!docs[0].v || docs[0].v != "2") {
                        ...
                        if (options.v1 ) {
                            result.members = transform.getV1ConceptDescriptors(result.members);
                        }
                        callback(false, result);
                    }
                }
