        ...
        db.open(function(err, db) {
            db.authenticate(config.db.username, config.db.password, function() {
                db.collection(req.params.collection + 'tx', function(err, collection) {
                    collection.find(query, options, function(err, cursor) {
                        cursor.toArray(function(err, docs) {
                            logger.log('info', 'Search for ' + searchTerm + ' result = 0', {searchTerm: searchTerm, database: req.params.db, collection: req.params.collection, searchMode: searchMode, language: lang, matches: docs.length});
                            var result = [];
                            if (docs.length > 0) {
                                docs.forEach(function(doc) {
                                    if (req.params.sctid) {
        ...
        db.open(function(err, db) {
            db.authenticate(config.db.username, config.db.password, function() {
                db.command({text: req.params.collection + 'tx', search: searchTerm, language: lang, filter: {"active": true, "conceptActive": true}}, function(e, cb) {
                    if (e) {
                        console.log(e, 'error');
                        logger.log('error', 'Search for ' + searchTerm + ' result = 0', {searchTerm: searchTerm, database: req.params.db, collection: req.params.collection, searchMode: searchMode, language: lang});
                        res.send(501);
                    } else {
                        //console.log("cb: " + JSON.stringify(cb));
                        logger.log('info', 'Search for ' + searchTerm + ' result = 0', {searchTerm: searchTerm, database: req.params.db, collection: req.params.collection, searchMode: searchMode, language: lang, matches: cb.results.length});
                        var result = [];
