                                    ...
                                    doc.descriptions.forEach(function(desc) {
                                        if (req.params.sctid) {
                                            ...
                                            if (desc.term.substring(0, searchTerm.length) == searchTerm.substring(0, searchTerm.length)) {
                                                result.push({"term": desc.term, "conceptId": desc.conceptId, "active": desc.active, "fsn": doc.defaultTerm});
                                            }
                                        }
                                    });
                                });
                    ...
                    collection.find(query, options, function(err, cursor) {
                        cursor.toArray(function(err, docs) {
                            ...
                            if (docs.length > 0) {
                                ...
                                matchedDescriptions.forEach(function(doc) {
                                    result.push({"term": doc.term, "conceptId": doc.conceptId, "active": doc.active, "fsn": doc.obj.fsn});
                                });
                                res.header('Content-Type', 'application/json');
                                res.send(result);
                            } else {
