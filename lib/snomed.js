...
app.get('/:db/:collection/descriptions/:sctid?', function(req, res) {
    ...
    if (req.params.sctid) {
        ...
        if (req.query["query"]) {
            ...
            } else if (req.query["searchMode"] == "fullText") { // TODO: bypassed at the end
                searchMode = req.query["searchMode"];
                searchTerm = req.query["query"];
                if (statusFilter == 'inactiveOnly') {
                    query = {text: req.params.collection + 'tx', search: searchTerm, limit: 1000000,language: lang, filter: {"active": false, "conceptActive": false}};
                } else if (statusFilter == 'activeAndInactive') {
                    query = {text: req.params.collection + 'tx', search: searchTerm, limit: 1000000,language: lang};
                } else {
                    query = {text: req.params.collection + 'tx', search: searchTerm, limit: 1000000,language: lang, filter: {"active": true, "conceptActive": true}};
                }
    ...
    if (searchMode == "regex" || searchMode == "partialMatching")  {
                                    ...
                                    matchedDescriptions.forEach(function(doc) {
                                        if (count >= skipTo && count < (skipTo + returnLimit)) {
                                            if (semanticFilter == "none" || (semanticFilter == doc.semanticTag)) {
                                                ...
                                                if (result.filters.lang.hasOwnProperty(doc.language)) {
                                                    result.filters.lang[doc.language] = result.filters.lang[doc.language] + 1;
                                                } else {
                                                    result.filters.semTag[doc.language] = 1;
                                                }
        ...
        db.open(function(err, db) {
            db.authenticate(config.db.username, config.db.password, function() {
                db.command(query, function(e, cb) {
                    var dbDuration = Date.now() - start;
                    if (e) {
                        console.log(e, 'error');
                        var duration = Date.now() - start;
                        ...
                        if (matchedDescriptions.length > 0) {
                            ...
                            matchedDescriptions.forEach(function(doc) {
                                if (count >= skipTo && count < (skipTo + returnLimit)) {
                                    ...
                                    if (result.filters.lang.hasOwnProperty(doc.language)) {
                                        result.filters.lang[doc.obj.language] = result.filters.lang[doc.obj.language] + 1;
                                    } else {
                                        result.filters.semTag[doc.obj.language] = 1;
                                    }
