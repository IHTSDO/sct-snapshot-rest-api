...
app.get('/:db/:collection/concepts/:sctid?', function(req, res) {
    ...
    var db = new mongo.Db(req.params.db, new mongo.Server(config.db.host, config.db.port, {'auto_reconnect': true, w:1}));
...
app.get('/:db/:collection/descriptions/:sctid?', function(req, res) {
        ...
        db.open(function(err, db) {
            db.authenticate(config.db.username, config.db.password, function() {
                db.collection(req.params.collection + 'tx', function(err, collection) {
                    collection.find(query, options, function(err, cursor) {
                        cursor.toArray(function(err, docs) {
                            logger.log('info', 'Search for ' + searchTerm + ' result = ' + docs.length, {searchTerm: searchTerm, database: req.params.db, collection: req.params.collection, searchMode: searchMode, language: lang, matches: docs.length});
                            var result = [];
                            if (docs.length > 0) {
                                docs.forEach(function(doc) {
                                    if (req.params.sctid) {
                                        if (idParam == doc.descriptionId) {
                                            result.push(doc);
                                        }
                                    } else if (searchTerm) {
        ...
        db.open(function(err, db) {
            db.authenticate(config.db.username, config.db.password, function() {
                db.command({text: req.params.collection + 'tx', search: searchTerm, language: lang, filter: {"active": true, "conceptActive": true}}, function(e, cb) {
                    if (e) {
                        console.log(e, 'error');
                        logger.log('error', 'Search for ' + searchTerm + ' ERROR', {searchTerm: searchTerm, database: req.params.db, collection: req.params.collection, searchMode: searchMode, language: lang});
                        res.send(501);
                    } else {
                        //console.log("cb: " + JSON.stringify(cb));
                        logger.log('info', 'Search for ' + searchTerm + ' result = ' + cb.results.length, {searchTerm: searchTerm, database: req.params.db, collection: req.params.collection, searchMode: searchMode, language: lang, matches: cb.results.length});
                        var result = [];
    ...
    } else if (searchMode == "wordsAnyOrder") {
        var options = req.params.options || {};
        var test = ['limit', 'sort', 'fields', 'skip', 'hint', 'explain', 'snapshot', 'timeout'];
        for (o in req.query) {
            if (test.indexOf(o) >= 0) {
