...
var getObject = function(dbP, collectionP, query, options, callback) {
    console.log("DB: " + JSON.stringify(dbP));
    console.log("Collection: " + JSON.stringify(collectionP));
                    ...
                    if (form == "inferred" && doc.relationships) {
                        doc.relationships.forEach(function(rel) {
                            if (rel.active == true && (rel.type.conceptId == 116680003 || rel.type.conceptId == "116680003")) {
                                result.push({ conceptId: rel.target.conceptId, defaultTerm: rel.target.defaultTerm, definitionStatus: rel.target.definitionStatus, module: rel.target.module, statedDescendants: rel.target.statedDescendants, inferredDescendants: rel.target.inferredDescendants });
                            }
                        });
